...
class RestClient:
	...
	def auth(self):
		...
		if self.allow_unsafe_url == True:
			...
			response = requests.post("{}/api/auth".format(self.api_url, ), verify=False, json=data, headers=self.headers)
		else:
			response = requests.post("{}/api/auth".format(self.api_url, ), verify=True, json=data, headers=self.headers)
